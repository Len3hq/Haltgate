...
- [x] **Full halt cycle demonstrated on-chain, 2026-09-20.** The §9 submission criterion end to end on the live SPY market, not a unit test: supplied 0.02 wSPYx and borrowed 5 USDG while OPEN; paused the oracle and called the permissionless `sync()`, moving the market to HALTED; confirmed `supply`, `borrow`, `withdraw` and `liquidate` **all revert with `MarketHalted` (`0x8fe89dfc`)**; then **repaid 5 USDG while still HALTED**, debt to zero, state unchanged at 2. Resumed the oracle, `sync()` to RESUMING where `canLiquidate` is still false (the grace window doing its job), `completeResume()` to OPEN, and withdrew the collateral. Repayment-never-blocks was the protocol's headline claim and until now had only ever been proven in tests.
...
- [x] Frontend, first pass: a **Fixed** tab in `DashboardTabs` alongside the variable-rate Borrow flow. Superseded, see the product-routes item below. Term selector, live `quoteFixed` preview of the exact total repayable at maturity, per-market LTV cap surfaced in the input, a countdown and a past-due state on the open loan, and copy that states the no-liquidation tradeoff in both directions. `settleMatured()` is surfaced in the **Liquidate** tab, which is where the third-party keeper role already lives.
...
- [x] **Promoted Fixed Term and Multiply from tabs to products, 2026-09-19.** Burying a differentiated product in the fifth tab of a shared page undersells it, and the cost was concrete rather than cosmetic: a tab has no URL so it can't be linked or demoed directly, and sharing one market table meant the per-asset fixed LTV (30% to 45%) had nowhere to live except a tooltip.
  - New routes: `/app/fixed` and `/app/multiply` (product page plus their own market list), `/app/fixed/[market]` and `/app/multiply/[market]` (the flows). Next.js resolves static segments ahead of `[market]`, and `generateStaticParams` only emits the five market keys, so there is no collision. 22 routes prerender.
  - `FixedMarketsTable` carries the column the shared table couldn't: max LTV per asset, plus the fixed rate and term range. `MultiplyMarketsTable` leads with each market's real ceiling, derived live from its own `maxLTV` rather than the contract's 5x cap that no market here approaches.
  - `ProductNav` in `AppShell` makes the three surfaces legible before anyone clicks. `ProductMarketShell` is shared chrome (provider, halt banner, price chart, in-product asset switcher) so the two product pages don't drift.
  - `DashboardTabs` drops to Borrow / Earn / Liquidate, which genuinely are three views of one variable-rate position, and cross-links to the two products for the same asset.
...
- [x] **Full security audit, 2026-09-21.** Every contract read line by line, plus tooling, live-chain state, operational security and a frontend review. Nine findings.
  - **Two unbounded setters in `Market`.** `setLiquidationBonus` and `setMaxOracleStaleness` had no ceilings while every other tunable had one. An unbounded bonus lets a dust repayment seize a whole position (the clamp stops the underflow, not the theft); an unbounded staleness either disables the freshness backstop or bricks pricing. Added `MAX_LIQUIDATION_BONUS = 20%`, `MIN/MAX_ORACLE_STALENESS = 1 hour / 7 days`.
  - **Unbounded rate parameters in `InterestRateModel`.** `kink` was bounded from the start, the three rate legs were not, on the one contract shared by all five markets. Added `MAX_RATE_PER_YEAR = 1000%`.
  - **The timelock was bypassable.** Every risk contract sat behind it, but the collateral tokens were still owned by a bare EOA whose `mint()` is `onlyOwner`. A compromised key could mint collateral, supply it and borrow out every pool with no delay. All five transferred to the multisig.
  - **Zero property tests.** 305 tests, all hand-written cases. Added 7 fuzz properties (LTV ceilings, round-trip value conservation, accounting integrity, liquidation bounds, quote monotonicity) plus 6 rate-model bound tests. The fuzzer immediately found the new `setRiskParams` guard by trying to lower `maxLTV` beneath the standing `fixedMaxLTV`.
  - **Docs contradicted the contract.** The halt table claimed interest froze in all four non-OPEN states. `interestFrozen()` returns true only for `HALTED` and `SETTLING`; HALTING and RESUMING keep accruing, which is correct since a usable price exists in both. Docs corrected, contract unchanged.
  - **`LeverageZap` left standing approvals.** `market` is caller-supplied and only checked for a matching token pair, so any surviving allowance was one an arbitrary address could draw on. Approvals now zeroed after every use, with tests asserting no residue and no allowance.
  - **`MockWrappedXStock.valueOf()` is not consulted by the protocol** and now says so in NatSpec. This is correct rather than a bug: the oracle's convention is to price the collateral token directly, so reading the rate as well would double-count. Raw xStocks rebase via a Multiplier; **wrapped** ones do not, which is exactly why the wrapper is the right collateral.
  - **Confirmed sound:** timelock correctly uses the open-executor pattern (`address(0)` holds `EXECUTOR_ROLE`, admin renounced to the timelock itself); the live key is in no tracked file, nowhere in git history and not in the frontend bundle; no lingering token approvals anywhere; `liquidate()` ordering, clamps and gating are correct; `withdrawReserves` cannot reach lender principal; `SwapModule` staleness is a private constant with immutable refs.
