...
- [x] **Design decisions, all confirmed before writing code:**
  ...
  - **Fixed-term LTVs are tiered per asset, all below the 50% variable cap.** A liquidatable position can be closed at the first sign of trouble; a fixed-term one must survive the whole term untouched, so an identical cap would carry materially more risk. Set from how far each asset realistically moves over a 30-day window rather than picked: **TSLA 30%, NVDA 35%, AAPL 40%, MSFT 40%, SPY 45%** (a diversified index, not a single name). Rate is **8%/yr simple** on all five; terms **1 to 30 days**, offered as 1 / 7 / 14 / 30.
